...
Original file is located at
    https://colab.research.google.com/drive/1R-7yrJI1MiQ_VM7cJFlDK031e1APn8Fy
"""
...
# =========================================================
# GRU ONLY ABLATION MODEL
# (Removes Graph Convolution, keeps Temporal Modeling)
# =========================================================
class GRUOnly(nn.Module):
    def __init__(
        ...
        # Directly feed raw node features into GRU
        self.gru = nn.GRU(
            input_size=in_feats,
    ...
    def forward(
        ...
        seq_edge_index: List = None,
        seq_edge_weight: List = None
    ):
        """
        seq_feats shape:
        (batch, seq_len, N, F)

        Graph inputs are ignored in this ablation.
        """
        batch, seq_len, N, F_dim = seq_feats.shape

        # Rearrange to GRU expected shape:
        # (seq_len, batch*N, F)
        seq_feats = seq_feats.permute(1, 0, 2, 3)
        seq_flat = seq_feats.reshape(seq_len, batch * N, F_dim)

        # Temporal modeling only
        gru_out, _ = self.gru(seq_flat)

        # Use last time step
        last = gru_out[-1]  # (batch*N, hidden)
...
def full_news_demo(
    ...
    in_feats = dataset.features.shape[-1]
    print(f"in_feats={in_feats}, dataset length={len(dataset)}")
    model = GRUOnly(in_feats=in_feats, gru_hidden=64, out_dim=1, dropout=0.2, mode="regression").to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-5)
    loss_fn = nn.MSELoss()
...
# =========================================================
# GCN ONLY ABLATION MODEL
# (Keeps Spatial Modeling, Removes Temporal GRU)
# =========================================================
class GCNOnly(nn.Module):
    def __init__(
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(gcn_hidden // 2, out_dim),
        )
    ...
    def forward(
        ...
        GCN Only uses only the last time step features.
        """
        batch, seq_len, N, F_dim = seq_feats.shape
        device = seq_feats.device

        # Use only last day features (no temporal modeling)
        x_last = seq_feats[:, -1, :, :]  # (batch, N, F)

        batch_outputs = []

        for b in range(batch):

            x = x_last[b].to(device)

            # Use last day graph
            ei_np = seq_edge_index[b][-1]
            ew_np = seq_edge_weight[b][-1]
            ...
            out = self.mlp(h)  # (N, out_dim)

            batch_outputs.append(out)

        preds = torch.stack(batch_outputs, dim=0)  # (batch, N, out_dim)
...
def full_news_demo(
    ...
    in_feats = dataset.features.shape[-1]
    print(f"in_feats={in_feats}, dataset length={len(dataset)}")
    model = GCNOnly(in_feats=in_feats, gcn_hidden=64, out_dim=1, dropout=0.2, mode="regression").to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-5)
    loss_fn = nn.MSELoss()
...
class TGCN(nn.Module):
    def __init__(
        ...
        dropout: float = 0.2,
        mode: str = "regression",
        static_graph: bool = True   # <-- NEW FLAG
    ):
        super().__init__()
    ...
    def forward(
        ...
        for t in range(seq_len):
            ...
            for b in range(batch):
                ...
                if self.static_graph:
                    ei_np = seq_edge_index[b][0]
                    ew_np = seq_edge_weight[b][0]
                else:
                    ei_np = seq_edge_index[b][t]
                    ew_np = seq_edge_weight[b][t]
        ...
        seq_stack = torch.stack(gcn_outputs, dim=0)
